...
class ConvVAE(object):
    def __init__(self, z_size=32, batch_size=1, learning_rate=0.0001, kl_tolerance=0.5, is_training=False, reuse=False, gpu_mode=False):
        ...
        with tf.variable_scope('conv.env', reuse=self.reuse):
            if not gpu_mode:
    ...
    def build_graph(self):
        ...
        with self.g.as_default():
            ...
            # Building the Encoder part of the VAR
            ...
            # Building the V part of VAE
            self.mu = tf.layers.dense(h, self.z_size, name='enc_fc_mu')
            self.logvar = tf.layers.dense(h, self.z_size, name='enc_fc_logvar')
            self.sigma = tf.exp(self.logvar) / 2.0
            self.epislon = tf.random_normal([self.batch_size, self.z_size])
            self.z = self.mu + self.sigma * self.epislon
            ...
            if self.is_training:
                ...
                self.train = self.optimizer.apply_gradients(grads, global_step=self.global_step, name='train_step')
