...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    val retBoundaries = new Boundaries()
    retBoundaries.minX = math.min(boundaries.minX, body.x)
    retBoundaries.maxX = math.max(boundaries.maxX, body.x)
    retBoundaries.minY = math.min(boundaries.minY, body.y)
    retBoundaries.maxY = math.max(boundaries.maxY, body.y)
    retBoundaries
  }

  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val retBoundaries = new Boundaries()
    retBoundaries.minX = math.min(a.minX, b.minX)
    retBoundaries.maxX = math.max(a.maxX, b.maxX)
    retBoundaries.minY = math.min(a.minY, b.minY)
    retBoundaries.maxY = math.max(a.maxY, b.maxY)
    retBoundaries
  }
  ...
  def accumulateBodies(quad: Quad)(newBods: Seq[Body], newBod: Body): Seq[Body] = {
    newBods :+ newBod.updated(quad)
  }

  def updateBodies(bodies: Seq[Body], quad: Quad): Seq[Body] = timeStats.timed("update") {
    val accumAndUpdateBodies = accumulateBodies(quad) _
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.aggregate(Seq[Body]())(accumAndUpdateBodies, (a,b) => a ++ b)
  }
