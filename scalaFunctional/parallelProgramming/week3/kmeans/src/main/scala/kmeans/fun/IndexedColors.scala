...
//  println(s"numPoints: ${points.size}, numMeans:  ${means.size}")
//  means.foreach(m => println(s"..... Mean(${m.x}, ${m.y}, ${m.z})"))
  ...
  private def imageToPoints(img: Img): GenSeq[Point] = {
    val points = new mutable.ParArray[Point](img.height * img.width)
    for (x <- 0 until img.width; y <- 0 until img.height) {
      val rgba = img(x, y)
      points.update(x + y * img.width, new Point(red(rgba), green(rgba), blue(rgba)))
    }
    points.par
  }
